...
export const createAccessToPathProxy = <T>(onNewPath: (path: Array<string>) => string): T => {
  const validator: ProxyHandler<T & ProxyStateExtension> = {
    get(target: T & ProxyStateExtension, p: string | symbol): unknown {
      ...
      return new Proxy<T & ProxyStateExtension>(
        { ...target, [InternalValues]: { path: [...path, p], depth: internalValues.depth + 1 } },
        validator,
      )
    },
  }
  ...
  return new Proxy<T & ProxyStateExtension>(
    { [InternalValues]: { path: [] as Array<string>, depth: 0 } } as T & ProxyStateExtension,
    validator,
  )
}
